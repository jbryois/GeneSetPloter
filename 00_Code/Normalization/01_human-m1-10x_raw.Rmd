...
```{r}
file_name='../../01_Processed/human-m1-10x.sum.txt'
```
...
select(exp,Gene,gene_id,entrez_id,Lvl5,Expr_sum_mean_scaled10k) %>% 
  ...
  write_tsv(gzfile("../../Data/AB_human_m1_10x.norm.txt.gz"))
```

```{r}
select(exp,Gene,gene_id,entrez_id,Lvl5,spe_10k) %>% 
  spread(Lvl5,spe_10k) %>% 
  write_tsv(gzfile("../../../BrainRich2/Data/AB_human_m1_10x.norm.txt.gz"))
```
