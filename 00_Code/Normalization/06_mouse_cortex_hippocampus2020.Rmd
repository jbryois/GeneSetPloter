...
```{r}
file_name='../../01_Processed/mouse_whole_cortex_hippocampus_2020.csv'
```
...
```{r}
exp <- exp %>% group_by(Gene) %>% mutate(spe_10k=Expr_sum_mean_scaled10k/sum(Expr_sum_mean_scaled10k))  %>% ungroup()
```
...
```{r}
m2h <- read_tsv('../../00_Raw/m2h.txt') %>% select(entrez,musName) %>%  rename(entrez_id=entrez) %>% mutate(entrez_id=as.character(entrez_id))
```

```{r}
entrez_ensembl <- inner_join(entrez_ensembl,m2h,by='entrez_id')
```

```{r}
exp <- rename(exp,musName=Gene)
```

```{r}
exp <- inner_join(exp,entrez_ensembl,by='musName')
```
...
select(exp,Gene,gene_id,entrez_id,Lvl5,Expr_sum_mean_scaled10k) %>% 
  ...
  write_tsv(gzfile("../../Data/AB_whole_cortex_hippocampus_mouse_2020.norm.txt.gz"))
```

```{r}
select(exp,Gene,gene_id,entrez_id,Lvl5,spe_10k) %>% 
  spread(Lvl5,spe_10k) %>% 
  write_tsv(gzfile("../../../BrainRich2/Data/AB_whole_cortex_hippocampus_mouse_2020.norm.txt.gz"))
```
